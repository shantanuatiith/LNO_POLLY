...
public:
  ...
  void PrintAccessFunction() {
    for(unsigned i = 0; i < AccessFunctions.size(); i++) {
      errs() << AccessFunctions[i].get() << "---- isl_id = "
             << isl_id_to_str(AccessFunctions[i].get()->getId()) << " ---- ";
      AccessFunctions[i].get()->getAccessInstruction()->dump();
    }
  }
