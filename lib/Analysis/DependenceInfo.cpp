...
void Dependences::calculateDependences(Scop &S) {
  ...
  if (isl_ctx_last_error(IslCtx.get()) == isl_error_quota) {
    ...
    isl_union_map_free(WRAP_RAW);
    isl_union_map_free(WRAP_WAW);
    isl_union_map_free(WRAP_WAR);
    WRAP_RAW = WRAP_WAW = WRAP_WAR = nullptr;
  ...
  WRAP_RAW = isl_union_map_copy(RAW);
  WRAP_WAW = isl_union_map_copy(WAW);
  WRAP_WAR = isl_union_map_copy(WAR);
  //WRAP_RED = isl_union_map_copy(RED);
  //WRAP_TC_RED = isl_union_map_copy(TC_RED);
...
void Dependences::releaseMemory() {
  ...
  isl_union_map_free(WRAP_WAW);
	...
	WRAP_RAW = WRAP_WAR = WRAP_WAW = nullptr;
...
__isl_give isl_union_map *Dependences::getWrappedDependences(int Kinds) const {
  assert(hasValidDependences() && "No valid dependences available");
  isl_space *Space = isl_union_map_get_space(WRAP_RAW);
  isl_union_map *Deps = isl_union_map_empty(Space);

  if (Kinds & TYPE_RAW)
    Deps = isl_union_map_union(Deps, isl_union_map_copy(WRAP_RAW));

  if (Kinds & TYPE_WAR)
    Deps = isl_union_map_union(Deps, isl_union_map_copy(WRAP_WAR));

  if (Kinds & TYPE_WAW)
    Deps = isl_union_map_union(Deps, isl_union_map_copy(WRAP_WAW));
/*
  if (Kinds & TYPE_RED)
    Deps = isl_union_map_union(Deps, WRAP_RED);

  if (Kinds & TYPE_TC_RED)
    Deps = isl_union_map_union(Deps, WRAP_TC_RED);
*/
  //Deps = isl_union_map_coalesce(Deps);
  Deps = isl_union_map_detect_equalities(Deps);
  return Deps;
